...
xfun::install_github("harperzhu/projectpackage")
...
```
*1*For the first t-test, we observe P = 0.09322877. Therefore we conclude that we have sufficient evidence to reject the null hypothesis and conclude that the true mean life expectancy is not equal to 60.

*2*For the second t-test, we observe P = 0.9533856 Therefore we conclude that we do not have sufficient evidence to reject the null hypothesis. Therefore, we cannot conclude that the true mean life expectancy is smaller than 60.

*3*For the third t-test, we observe P = 0.04661438. Therefore we conclude that we  have sufficient evidence to reject the null hypothesis and conclude that the true mean life expectancy is greater than 60.
```{r}
...
#Calculating cv_err and training_err for 10 times.
for(i in 1:10) {
...
*1*According to the results in the table we can see that when k_nn = 1, we have both the smallest cv error and the training error, so I think k_nn = 1 is the best model when k_cv = 5. 

*2*Cross validation is a statistical method that used to check how accurately our model can predict the result. This method is very useful since it test the model multiple time by using different parts of the same dataset, which is vert efficient.
